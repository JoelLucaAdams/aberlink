...
class postgreSQL():
    ...
    def get_discord_user(discord_id: int):
        ...
        row = cur.fetchone()
        try:
            message = {"id": row[0], "last_login": row[1], "openidc_id": row[2]}
            return message
        except TypeError:
            return None
    ...
    def get_openid_user(openidc_id: int):
        ...
        row = cur.fetchone()
        try:
            message = {"id": row[0], "username": row[1], "name": row[2], "email": row[3], "usertype": row[4], "last_login": row[5], "is_active": row[6], "is_admin": row[7]}
            return message
        except TypeError:
            return None
